...
const map = {
  ...
  render(snakePointsArray, foodPoint, pointRock) {
    ...
    const rockCell = this.cells[`x${pointRock.x}_y${pointRock.y}`];
...
const snake = {
  ...
  isOnRock(pointRock) {
    return this.bode.some((snakePoint) => snakePoint.x === pointRock.x && snakePoint.y === pointRock.y)
  },
...
const rocks = {
  ...
  isOnRock(pointRock) {
    return this.x === pointRock.x && this.y === pointRock.y;
  }
...
const game = {
  ...
  countRender() {
    ...
    checker.innerHTML = "Сделано " + this.countChecker + " попыток";
  },

  winRender() {
    const winChecker = document.querySelector('.win-checker');
    winChecker.innerHTML = "Текущее количество побед - " + this.winChecker;
  },
  ...
  reset() {
    ...
    this.rocks.setRocks(this.getRandomFreeCoordinates());
    this.render();
  },
  ...
  getRandomFreeCoordinates() {
    const exclude = [this.food.getCoordinates(), ...this.snake.getBody(), this.rocks.getRocks()];
    // without ... -  [{}, [{}, {}, {}]] => with ... [{}, {}, {}, {}];
    while (true) {
      const rndPoint = {
  ...
  render() {
    this.map.render(this.snake.getBody(), this.food.getCoordinates(), this.rocks.getRocks());
  },
  ...
  finish() {
    ...
    clearInterval(this.tickInterval);
    this.setPlayButton('Вы выиграли', true);
    this.winCheck();
  },
  ...
  tickHandler() {
    if (!this.canMakeStep()) return this.fail();

    if (this.rocks.isOnRock(this.snake.getNextStepHeadPoint())) return this.fail();
